          ...
          <>
            ...
            {cart.length > 0 && (
              <div className="px-7 py-5 border-t border-stone flex-shrink-0">
                ...
                <button
                  onClick={() => {
                    if (!user) { onClose(); navigate('/connexion'); }
                    else { setStep('form'); }
                  }}
                  className="w-full bg-dark text-white text-xs tracking-widest uppercase py-3"
                >
                  Commander
                </button>
                {!user && (
                  <p className="text-[11px] text-muted text-center mt-3">
                    <Link to="/connexion" onClick={onClose} className="text-accent hover:underline">Connectez-vous</Link>
                    {' '}ou{' '}
                    <Link to="/inscription" onClick={onClose} className="text-accent hover:underline">créez un compte</Link>
                    {' '}pour commander
                  </p>
                )}
              </div>

  ...
  return (
    <div onClick={onClose} className="fixed inset-0 bg-dark/60 z-50 flex items-center justify-center p-6">
      <div onClick={e => e.stopPropagation()}
        ...
        <ProductImage src={product.image_url} alt={product.name} className="w-full aspect-[3/4]" />
        <div className="p-10 flex flex-col justify-between">
          ...
          <div className="mt-8">
            ...
            <button
              onClick={handleAdd}
              disabled={added || !product.in_stock || (product.sizes?.length > 0 && !selectedSize)}
              className={`w-full py-3 text-[12px] tracking-[0.08em] uppercase transition-colors disabled:cursor-not-allowed ${
                added
                  ? 'bg-green-700 text-white'
                  : 'bg-dark text-white hover:bg-dark/90 disabled:opacity-40'
              }`}>
              {added ? '✓ Ajouté au panier' : product.in_stock ? 'Ajouter au panier' : 'Épuisé'}
            </button>

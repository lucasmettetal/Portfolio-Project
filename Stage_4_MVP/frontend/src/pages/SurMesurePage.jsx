...
export default function SurMesurePage() {
  ...
  async function handleSubmit(e) {
    ...
    if (!consent) {
      setError('Vous devez accepter la politique de confidentialité pour soumettre votre demande.');
      return;
    }
    setLoading(true);
    setError('');
        ...
        <>
          ...
          <section className="max-w-4xl mx-auto px-4 md:px-10 pb-20">
            <form onSubmit={handleSubmit} className="grid md:grid-cols-2 gap-x-12 gap-y-10">
              ...
              {/* Submit */}
              <div className="md:col-span-2 border-t border-stone pt-8 flex flex-col gap-5">
                <label className="flex items-start gap-3 cursor-pointer">
                  <input type="checkbox" checked={consent} onChange={e => setConsent(e.target.checked)}
                    className="mt-0.5 w-4 h-4 accent-accent flex-shrink-0" />
                  <span className="text-xs text-muted leading-relaxed">
                    J'accepte que mes données personnelles et mes mensurations soient traitées par Rue 25 dans le but de gérer ma demande sur-mesure, conformément à la{' '}
